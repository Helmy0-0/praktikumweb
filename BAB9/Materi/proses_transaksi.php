...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
   ...
   try {
       ...
       $stmt = $con->prepare("INSERT INTO Pesanan (Tanggal_Pesanan, Pelanggan_ID, Total_Harga) VALUES (?, ?, ?)");
       ...
       foreach ($_POST['buku'] as $buku) {
           ...
           $stmt = $con->prepare("SELECT Harga, Stok FROM Buku WHERE ID = ?");
           ...
           // Insert detail
           $stmt = $con->prepare("INSERT INTO Detail_Pesanan (Pesanan_ID, Buku_ID, Kuantitas, Harga_Per_Satuan) VALUES (?, ?, ?, ?)");
           ...
           // Update stok
           $stmt = $con->prepare("UPDATE Buku SET Stok = Stok - ? WHERE ID = ?");
       ...
       // Update total harga
       $stmt = $con->prepare("UPDATE Pesanan SET Total_Harga = ? WHERE ID = ?");
